...
*/
    ...
    /* TCP */    
	Modbus ctx = Modbus("127.0.0.1", 1502);
	ctx.set_debug(true);
    ...
    while (nb_loop++ < LOOP) {
        for (addr = ADDRESS_START; addr < ADDRESS_END; addr++) {
            ...
            rc = ctx.modbus_write_bit(addr, tab_rq_bits[0]);
            if (rc != 1) {
                printf("ERROR modbus_write_bit (%d)\n", rc);
                printf("Address = %d, value = %d\n", addr, tab_rq_bits[0]);
                nb_fail++;
            } else {
                rc = ctx.modbus_read_bits(addr, 1, tab_rp_bits);
                rc = ctx.modbus_read_bits(addr, 1, tab_rp_bits);
            ...
            /* MULTIPLE BITS */
            rc = ctx.modbus_write_bits(addr, nb, tab_rq_bits);
            if (rc != nb) {
                printf("ERROR modbus_write_bits (%d)\n", rc);
                printf("Address = %d, nb = %d\n", addr, nb);
                nb_fail++;
            } else {
                rc = ctx.modbus_read_bits(addr, nb, tab_rp_bits);
                if (rc != nb) {
            ...
            /* SINGLE REGISTER */
            rc = ctx.modbus_write_register(addr, tab_rq_registers[0]);
            if (rc != 1) {
                ...
                rc = ctx.modbus_read_registers(addr, 1, tab_rp_registers);
                if (rc != 1) {
            ...
            /* MULTIPLE REGISTERS */
            rc = ctx.modbus_write_registers(addr, nb, tab_rq_registers);
            if (rc != nb) {
                printf("ERROR modbus_write_registers (%d)\n", rc);
                printf("Address = %d, nb = %d\n", addr, nb);
                nb_fail++;
            } else {
                rc = ctx.modbus_read_registers(addr, nb, tab_rp_registers);
                if (rc != nb) {
